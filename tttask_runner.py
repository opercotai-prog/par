...
# Целевая дата (можно менять на текущую)
...
async def process_with_ai(text, city_context):
    """ИИ-Парсер: Gemini 2.0 Flash через прямой запрос"""
    # Используем модель 2.0-flash, которая у тебя работала
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={GEMINI_KEY}"
    
    prompt = f"""
    Верни JSON строго по схеме. Если данных нет - null.
    is_ad: true только если это ПРЕДЛОЖЕНИЕ жилья в г. {city_context}. 
    is_ad: false если это поиск ("сниму"), новости или мусор.
    ...
    for ch in res_ch.data:
        ...
        async for msg in client.iter_messages(ch['username'], offset_date=TARGET_DATE + timedelta(days=1), limit=50):
            ...
            supabase.table("eraw_posts").upsert({
                "channel_id": ch['id'],
                "tg_post_id": msg.id,
                "text": msg.text, # Простой текст без Markdown
                "status": status,
                "created_at": msg.date.isoformat()
            }, on_conflict="channel_id, tg_post_id").execute()

    # 2. ОБРАБОТКА ИИ (Твои рабочие лимиты)
    # Берем 5 постов за раз
    ...
    for post in res_new.data:
        ...
        if ai_data:
            ...
            if ai_data.get('is_ad') is True:
                ...
                # Ссылка на превью поста (самый простой способ фото)
